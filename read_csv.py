...
if __name__ == "__main__":
    ...
    LINE_NUMBER = 1000
    read_data = read_csv_line(FILE_PATH, LINE_NUMBER + 1)
    if FILE_PATH == 'hand_dataset.csv':
        if read_data is not None:
            ...
            plt.title('Original Hand')

            # rotated_data = add_random_noise(read_data[:-1])
            # rotated_hand_data = [0, 0, 0] + rotated_data.tolist()
            # fig2, ax2, lines2, scatters2 = create_plot()
            # show_plot(ax2, lines2, scatters2, rotated_hand_data)
            # plt.title('Processed Hand')
